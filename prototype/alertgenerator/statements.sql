 ...
 * in a amount of time which is not possible to travel in terms of distance.
 * In real world you have map tools and API to calculate distance. However this is outside of the scope of this project.
 * We will use a reference table with pre-defined time factor for distance.
 *
 */
create stream sourceData(transactionId string, accountId string, cardId string, txTime bigint, amount double, TxPostcode string, TxCountry string)
with(kafka_topic='sourceDataTopic', partitions=1, value_format='json', timestamp='txTime');

create table referenceTime(postcodes struct<postcodeA string, postcodeB string> primary key, travelTime bigint, timeMS bigint)
with(kafka_topic='referenceTimeTopic', partitions=1, value_format='json', key_format='json');

create table cardUsageDistance as select cardId, latest_by_offset(transactionId) transactionId, latest_by_offset(accountId) accountId,
...
create table alert as select concat(transactionId,cardId,accountId) alertID, 'Suspicious Account Activity' Alert_Type, 'Multiple Unusual Transactions' Breach_Type,
